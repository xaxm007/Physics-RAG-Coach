...
from memory.redis_chat import get_redis_history
...
async def lifespan(app: FastAPI):
    ...
    print("Redis session history cleared.")
...
async def query(
    ...
    try:
        ...
        general_template = template["template"]
        rephrase_template = collection.find_one({"name": "context prompt"})["template"]
        vector_store = request.app.state.vector_store
        response = rag_chain(user_query, vector_store, general_template, rephrase_template)
